...
function lsLoad() {
  try {
    ...
    const m = localStorage.getItem(LS.customMenu);
...
══════════════════════════════════════════ */
async function init() {
  // 1. Load from localStorage first — this is the source of truth until cloud responds
  lsLoad();

  // 2. Default stockQty for any item not yet in localStorage
  allItems().forEach(i => {
  ...
  if (DB_READY) {
    ...
    try {
      ...
      // Stock: merge cloud into local (cloud wins per-item)
      if (cloudStock && Object.keys(cloudStock).length) {
        Object.assign(stockQty, cloudStock);
      }

      // ─── FIX: Only overwrite salesData if cloud actually has records.
      // If cloudSales is null or empty, keep what we loaded from localStorage.
      if (cloudSales && Object.keys(cloudSales).length) {
        // Merge: prefer cloud values but keep local-only keys
        Object.assign(salesData, cloudSales);
      }

      // ─── FIX: Same for orders — only overwrite if cloud has records.
      if (cloudOrders && cloudOrders.length) {
        orderHistory = cloudOrders;
        orderCounter = (cloudOrders[0]?.id || 0) + 1;
      }

      if (cloudMenu) {
        customMenuItems = cloudMenu;
      }

      // Default stock for any new items from cloud menu
...
async function placeOrder(suffix) {
  ...
  order = [];
  lsSave(); // Save to localStorage immediately
...
function renderStock() {
  ...
  document.getElementById("stock-grid").innerHTML = items.map(i => {
    ...
    let status = "", right = "";
    const editBtn = `<button class="sq-btn" style="font-size:11px;width:auto;padding:0 8px;border-radius:6px" onclick="setStockModal('${esc(i.n)}')">✎</button>`;

    if (!tracks) {
      status = `<span class="stock-status-pill ok">Unlimited</span>`;
      right  = `<span style="font-size:11px;color:var(--tm);font-style:italic">Drink — no tracking</span>`;
    } else if (q === null) {
      status = `<span class="stock-status-pill ok">Not set</span>`;
      right  = `<button class="sq-btn" onclick="adjustStock('${esc(i.n)}',-1)">−</button><span class="sq-num">—</span><button class="sq-btn" onclick="adjustStock('${esc(i.n)}',1)">+</button>${editBtn}`;
    } else if (empty) {
      status = `<span class="stock-status-pill empty">Out of stock</span>`;
      right  = `<button class="sq-btn" onclick="adjustStock('${esc(i.n)}',-1)">−</button><span class="sq-num">${q}</span><button class="sq-btn" onclick="adjustStock('${esc(i.n)}',1)">+</button>${editBtn}`;
    } else if (low) {
      status = `<span class="stock-status-pill low">${q} remaining — LOW</span>`;
      right  = `<button class="sq-btn" onclick="adjustStock('${esc(i.n)}',-1)">−</button><span class="sq-num">${q}</span><button class="sq-btn" onclick="adjustStock('${esc(i.n)}',1)">+</button>${editBtn}`;
    } else {
      status = `<span class="stock-status-pill ok">${q} remaining</span>`;
      right  = `<button class="sq-btn" onclick="adjustStock('${esc(i.n)}',-1)">−</button><span class="sq-num">${q}</span><button class="sq-btn" onclick="adjustStock('${esc(i.n)}',1)">+</button>${editBtn}`;
    }
    ...
    return `<div class="${cls}">
      ...
      <div class="stock-right">${right}</div>
    </div>`;
  }).join("");
}
...
async function setStockModal(name) {
  const res = await showModal({
    ...
    title: "Set stock quantity", subtitle: name,
    body: `<p>Enter how many you have for this shift.</p>`,
    input: { placeholder: "e.g. 5", default: stockQty[name] !== null ? String(stockQty[name]) : "", type: "number", min: 0 },
    buttons: [
...
async function confirmResetShift() {
  const res = await showModal({
    icon: "↺", iconColor: "amber",
    title: "Reset shift stock?", subtitle: "Start of new shift",
    body: `<p>Sets all Sweets back to "not set" so you can enter fresh quantities.</p><p>Order history and analytics are <strong>not</strong> affected.</p>`,
    buttons: [
    ...
    <div class="log-card">
      <div class="log-top">
        ...
        <div class="log-meta">
          ...
          <button class="action-btn" style="padding:4px 10px;font-size:11px"
            onclick='showReceiptModalById(${JSON.stringify(JSON.stringify(o))})'>
...
async function endShift() {
  const res = await showModal({
    icon: "📊", iconColor: "green",
    title: "End shift", subtitle: "Print shift summary?",
...
async function clearAnalytics() {
  const res = await showModal({
    ...
    title: "Clear all analytics data?", subtitle: "This cannot be undone",
  ...
  if (mgmtShowInactive) {
    customMenuItems.filter(i => i.active === false).forEach(i => {
      if (!source.find(x => x.n === i.name)) {
        source.push({ n: i.name, p: parseFloat(i.price), cat: i.category,
          trackQty: i.track_qty, source: "db", id: i.id, inactive: true });
      }
    });
  }
        ...
        return `<div class="mgmt-item-row${isDead ? " mgmt-inactive" : ""}">
          ...
          <div class="mgmt-item-actions">
            ${isDead
              ...
              : `<button class="mgmt-btn edit"
                   onclick="openEditItem('${esc(item.n)}',${item.p},'${esc(item.cat)}',${item.trackQty},${isDB ? item.id : "null"})">
                   Edit
                 </button>
                 ${isDB
...
async function openAddItem() {
  ...
  const body = `
    <div class="mgmt-form">
      ...
      <div class="mgmt-field">
        <label>New category name (only if "New category" selected above)</label>
      ...
      <div class="mgmt-field mgmt-toggle-row">
        <label>Track stock quantity (for Sweets-like items)</label>
  ...
  const res = await showModal({
    icon: "➕", iconColor: "green",
    title: "Add new item", subtitle: "Appears on the order menu immediately",
    body,
    buttons: [
      { label: "Add item", value: "add",    style: "primary" },
      { label: "Cancel",   value: "cancel", style: "" }
    ]
  });
  ...
  if (dbMode) {
    try {
      const created = await dbCreateMenuItem({ name, price, category: cat, trackQty: track, sortOrder: 99 });
      // dbCreateMenuItem returns an array (Supabase POST returns array)
...
async function openEditItem(name, price, cat, trackQty, dbId) {
    ...
    <div class="mgmt-form">
      ...
      ${dbId === null
        ? `<p style="font-size:12px;color:var(--amber);margin-top:8px">
             ⚠️ Built-in item. Saving creates a cloud override.
           </p>`
        : ""}
    </div>`;
  ...
  const res = await showModal({
    ...
    body,
    buttons: [
      { label: "Save changes", value: "save",   style: "primary" },
      { label: "Cancel",       value: "cancel", style: "" }
    ]
  });
  if (res.action !== "save") return;
  ...
  if (dbMode) {
    try {
      if (dbId !== null) {
        ...
        await dbUpdateMenuItem(dbId, {
          name: newName, price: newPrice, category: newCat, track_qty: newTrack
        });
        ...
        // Built-in item — create a cloud override
        const created = await dbCreateMenuItem({
          name: newName, price: newPrice, category: newCat, trackQty: newTrack, sortOrder: 99
        });
        const newItem = Array.isArray(created) ? created[0] : created;
        customMenuItems.push(newItem);
      }
    ...
    if (idx > -1) {
      customMenuItems[idx] = { ...customMenuItems[idx], name: newName, price: newPrice, category: newCat, track_qty: newTrack };
    } else {
      customMenuItems.push({ id: Date.now(), name: newName, price: newPrice, category: newCat, track_qty: newTrack, active: true, sort_order: 99 });
    }
    lsSave(); buildCatTabs(); renderItems(); renderMenuMgmt();
...
async function deactivateMenuItem(dbId, name) {
  const res = await showModal({
    icon: "👁️", iconColor: "amber",
    title: `Hide "${name}"?`, subtitle: "Item will disappear from the menu",
    body: `<p>The item won't show in orders but history is preserved. You can restore it anytime from "Show hidden".</p>`,
    buttons: [
      { label: "Hide item", value: "yes", style: "danger" },
      { label: "Cancel",    value: "no",  style: "" }
    ]
  });
  if (res.action !== "yes") return;
...
async function overrideHideItem(name) {
  const res = await showModal({
    icon: "👁️", iconColor: "amber",
    title: `Hide "${name}"?`, subtitle: "Built-in item",
    body: `<p>This built-in item will be overridden in cloud and hidden from the menu. You can restore it anytime.</p>`,
    buttons: [
      { label: "Hide item", value: "yes", style: "danger" },
      { label: "Cancel",    value: "no",  style: "" }
    ]
  ...
  try {
    const created = await dbCreateMenuItem({
      name, price: baseItem.p, category: baseItem.cat,
      trackQty: baseItem.trackQty, sortOrder: 99
    });
    const newItem = Array.isArray(created) ? created[0] : created;
    await dbDeactivateMenuItem(newItem.id);
    customMenuItems.push({ ...newItem, active: false });
    lsSave(); buildCatTabs(); renderItems(); renderMenuMgmt();
...
async function hardDeleteMenuItem(dbId) {
  ...
  const res = await showModal({
    icon: "🗑️", iconColor: "red",
    title: `Permanently delete "${name}"?`, subtitle: "This cannot be undone",
    body: `<p>The item will be completely removed from the database. Sales history mentioning it will remain in the logs.</p>`,
    buttons: [
      { label: "Delete permanently", value: "yes", style: "danger" },
      { label: "Cancel",             value: "no",  style: "" }
    ]
  });
  if (res.action !== "yes") return;
